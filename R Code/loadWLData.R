...
tpPML<-tp[tp$depthClass=='PML'&tp$TP>0&tp$TP<100,c('dateTimeSample','TP')]
...
data<-evap
data<-data[as.Date(data$datetime)>as.Date('2014-03-01'),]
...
years<-c(2014,2015)
metab2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/Results/WL/WL_2014 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/Results/WL/WL_2014 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
metab2014<-merge(metab2014,gpp2014,by='solarDay',all.x=T)
metab2014<-metab2014[,c('solarDay','GPP','rhoEst','iotaEst','GPPSd','rhoSd','iotaSd')]
metab2014$NEP<-metab2014$GPP-metab2014$rhoEst
metab2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/WL/WL_2015 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/WL/WL_2015 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
...
metab<-rbind(metab2014,metab2015)
colnames(metab)=c('datetime','GPP','R','Iota','sdGPP','sdR','sdIota')
data<-merge(data,metab,by='datetime',all.x=T)
...
if(lake%in%c('EL','MO','HB','WL')){
  ...
  perim<-data.frame(lakeID=c('MO','EL','CR','HB','WL'),perim=c(1050.957,800.90,3455.0459,350,1311.1),vol=c(142454,123734,1302506,27007.52,189346.82),
                    wa=c(1122138,72003,319951,61748.94,92242))
  crData$overland<-crData$overland/perim$perim[perim$lakeID=='CR']*perim$perim[perim$lakeID==lake]*.3 # adjusted low to make res time longer # m3 / m shoreline / day 
  crData$overlandDOCdisch<-crData$overlandDOCdisch/perim$perim[perim$lakeID=='CR']*perim$perim[perim$lakeID==lake]*.3 # m3 / m shoreline / day 
  crData$overlandTPdisch<-crData$overlandTPdisch/perim$perim[perim$lakeID=='CR']*perim$perim[perim$lakeID==lake]*.3 # m3 / m shoreline / day 
  ...
  data$overland<-ifelse(is.na(data$overland),0,data$overland)
  data$overlandDOCdisch<-data$overland*26*1000
...
if(lake%in%c('HB','WL')){
  ...
  waterIn<-data$overland+data$precip # all water that goes into HB
  data$delta_storage<-ifelse(abs(data$delta_storage>2000),0,data$delta_storage)
  waterOut<-data$evap-data$delta_storage #water (except outlet) that leaves MO or change in storage 
  outlet<-waterIn-waterOut #difference between waterIn and evap+storage should be outlet discharge 
  data$outletDischarge<-outlet
...
}else if(lake%in%c('HB','WL')){
  data$totalAlloC<-(data$precipDOC+data$overlandDOCdisch)/lakeVol #total allochthonous carbon flux in mg C m-3 
  data$totalAlloP<-(data$precipTP+data$overlandTPdisch)/lakeVol # total allochthonous P flux in ug P m-3 
}
...
data$thermo.depth<-ifelse(is.na(data$thermo.depth),max(get.offsets(cur)),data$thermo.depth)
data$z.mix<-ifelse(is.na(data$z.mix),max(get.offsets(cur)),data$z.mix)
data$epiTemp[1]<-data$epiTemp[min(which(!is.na(data$epiTemp)))]
data$epiTemp[nrow(data)]<-data$epiTemp[max(which(!is.na(data$epiTemp)))]
data$epiTemp<-approx(1:nrow(data),data$epiTemp,1:nrow(data))$y
...
wlDeepholeCO2<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'WL deepHole point pCO2'&gc$depthTop<0.9&gc$CO2original_umolL>5&gc$CO2original_umolL<1000,]
wlDeepholeDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'WL deepHole point DIC'&gc$depthBottom<0.9&gc$CO2original_umolL>17,]
wlHypoDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                'WL deepHole point DIC'&gc$depthTop>3&gc$CO2original_umolL>6,]
stCO2<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
            'EL Inlet1 point pCO2'&gc$depthTop<0.9&gc$CO2original_umolL>40&gc$CO2original_umolL<100000&
            as.Date(gc$dateSample)>as.Date('2014-01-01'),]  # closest to overland CO2 
stDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
            'EL Inlet1 point DIC'&gc$depthTop<0.9&gc$CO2original_umolL>40&gc$CO2original_umolL<100000&
            as.Date(gc$dateSample)>as.Date('2014-01-01'),]  # closest to overland DIC 
aveStreamCO2=mean(stCO2$CO2original_umolL,na.rm = T)/1000/1000*1000 # umol L-1 to mol C m-3
aveStreamDIC=mean(stDIC$CO2original_umolL,na.rm=T)/1000/1000*1000 # umol L-1 to mol C m-3

wlDeepholeCO2<-aggregate(wlDeepholeCO2$CO2original_umolL~wlDeepholeCO2$dateSample,FUN=mean)
colnames(wlDeepholeCO2)<-c('datetime','co2') 
wlDeepholeCO2$co2<-wlDeepholeCO2$co2/1000/1000*1000 #converting from umol L-1 to mol C m-3
wlDeepholeDIC<-aggregate(wlDeepholeDIC$CO2original_umolL~wlDeepholeDIC$dateSample,FUN=mean)
colnames(wlDeepholeDIC)<-c('datetime','dic') 
wlDeepholeDIC$dic<-wlDeepholeDIC$dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
wlHypoDIC<-aggregate(wlHypoDIC$CO2original_umolL~wlHypoDIC$dateSample,FUN=mean)
colnames(wlHypoDIC)<-c('datetime','hypo_dic') 
wlHypoDIC$hypo_dic<-wlHypoDIC$hypo_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
data<-merge(data,wlDeepholeCO2,by='datetime',all.x=T)
data<-merge(data,wlDeepholeDIC,by='datetime',all.x=T)
data<-merge(data,wlHypoDIC,by='datetime',all.x=T)
...
#######################################################################i'm here 
data$co2In<-data$overland*aveStreamCO2 # m3 day-1 x mol C m-3 = mol C day-1 
data$co2In<-ifelse(is.na(data$co2In),0,data$co2In)
data$dicIn<-data$overland*aveStreamDIC+data$precipDIC/12/1000 # m3 day-1 x mol C m-3 = mol C day-1 
...
highFreqStaff<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/PressureSensorWaterHeight/WL/StaffGauge/WL_StaffGauge_waterHeight.txt',
                          stringsAsFactors = F,sep='\t',header=T)
highFreqStaff15<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/PressureSensorWaterHeight/WL/WholeLake/WL_WholeLake_waterHeight.txt',
                            stringsAsFactors = F,sep='\t',header=T)
...
data$waterLoad<-data$precip+data$overland # m3 day-1
data$waterLoss<-data$QoutInt+data$evap+data$gwDisch # m3 day-1
...
# sedimentation rates 
sedTrap2014<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Sediment Trap CNP/WL/WL_SedTrap.csv',stringsAsFactors = F)
sedTrap2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Sediment Trap CNP/WL/WL_SedTrap.csv',stringsAsFactors = F)
...
chl14<-chl14[chl14$lakeID==lake&chl14$depthClass=='PML'&chl14$flag==0&chl14$chl>0,]
...
chl15<-chl15[chl15$lakeID==lake&chl15$depthClass=='PML'&chl15$flag==0&chl15$chl>0,]
...
chl_hypo<-chl_hypo[chl_hypo$hypo_chl<40,]
