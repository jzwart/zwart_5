...
source('C:/Users/jzwart/Documents/Jake/R functions/DOY.r')
...
lakeArea<-as.numeric(bathy$area_m2[as.numeric(bathy$depth_m)==0])
if(lake=='MO'){
...
staff<-staff[,c(6,12)]
staff2014<-read.csv('/Users/jzwart/Documents/Jake/MyPapers/Pulses and Lags/Data/staffGaugeLog.csv',stringsAsFactor=F)
...
staff2015<-read.csv('E:/Jake/UNDERC 2015/Data/StaffGauges2015_1.csv',stringsAsFactors = F)
...
doc2014<-read.csv('/Users/jzwart/Documents/Jake/MyPapers/Pulses and Lags/Data/DOC2014.csv',stringsAsFactor=F)
...
doc2015<-read.csv('E:/Jake/UNDERC 2015/Data/Limnology/DOC2015.csv',stringsAsFactors = F)
...
poc2014<-read.table('E:/Jake/UNDERC 2014/Data/Limnology/POC_PON_2014.txt',stringsAsFactors = F,header = T,sep='\t')
...
poc2015<-read.csv('E:/Jake/UNDERC 2015/Data/Limnology/POC2015_samps.csv',stringsAsFactors = F)
...
airT<-read.csv('E:/Jake/UNDERC 2013/B3/Output/WL/WL2013/WL2013.csv',header=T)
...
airT2014<-read.table('E:/Jake/UNDERC 2014/LakeMetabolism/InputFiles/WL/airTemp.txt',stringsAsFactor=F,
                     sep='\t',header=T)
...
airT2015<-read.table('E:/Jake/UNDERC 2015/LakeMetabolism/InputFiles/Pelagic/WL/airTemp.txt',stringsAsFactors = F,sep='\t',header=T)
...
tp2014<-read.csv('E:/Jake/UNDERC 2014/Data/Limnology/TP//TP2014.csv',stringsAsFactors=F) #read in 2014 data
...
elDeepholeDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'EL deepHole point DIC'&gc$depthTop<0.9&gc$CO2original_umolL>5,]
...
colnames(elDeepholeCO2)<-c('datetime','co2')
elDeepholeCO2$co2<-elDeepholeCO2$co2/1000/1000*1000 #converting from umol L-1 to mol C m-3
elDeepholeDIC<-aggregate(elDeepholeDIC$CO2original_umolL~elDeepholeDIC$dateSample,FUN=mean)
colnames(elDeepholeDIC)<-c('datetime','dic')
elDeepholeDIC$dic<-elDeepholeDIC$dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
elHypoDIC<-aggregate(elHypoDIC$CO2original_umolL~elHypoDIC$dateSample,FUN=mean)
colnames(elHypoDIC)<-c('date
                       time','hypo_dic')
...
data<-merge(data,elDeepholeDIC,by='datetime',all.x=T)
...
data$co2In<-data$streamCO2disch+data$overland*aveStreamCO2 # mol C day-1
data$dicIn<-data$streamDICdisch+data$overland*aveStreamDIC+data$precipDIC/12/1000 # mol C day-1
...
# sedimentation rates
sedTrap2014<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Sediment Trap CNP/EL/EL_SedTrap.csv',stringsAsFactors = F)
sedTrap2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Sediment Trap CNP/EL/EL_SedTrap.csv',stringsAsFactors = F)
