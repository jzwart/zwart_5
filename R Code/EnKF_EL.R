...
parGuess <- c(0.004,0.3,0.30,0.1,0.01,0.06) #r20; units: day-1; fraction labile of loaded DOC; fraction inlet that goes into epi; turnover rate parameters set constant frac labile estimated 
...
for(i in 1:nEn){
  B[,,1,i]<-matrix(c(1-data2$QoutInt[1]/data2$epiVol[1]-data2$kCO2[1]/data2$thermo.depth[1]+
                ...
                0,0,0,0,1-data2$QoutInt[1]/data2$epiVol[1]-sedRateVec[i]+(data2$entrainVol[1]-data2$streamWaterdisch[1]*(1-splitFunc(data2$epiDens[1],data2$streamDens[1],fracInVec[i])))*data2$entrainHypo[1]/data2$epiVol[1],0,0,
                0,0,0,0,0,1-data2$QoutInt[1]/data2$epiVol[1]-sedRateVec[i]-mortVec[i]+(data2$entrainVol[1]-data2$streamWaterdisch[1]*(1-splitFunc(data2$epiDens[1],data2$streamDens[1],fracInVec[i])))*data2$entrainHypo[1]/data2$epiVol[1],0,
                0,0,0,0,1-data2$QoutInt[1]/data2$epiVol[1]-sedRateVec[i]+(data2$entrainVol[1]-data2$streamWaterdisch[1]*(1-splitFunc(data2$epiDens[1],data2$streamDens[1],fracInVec[i])))*data2$entrainHypo[1]/data2$epiVol[1],
                1-phyto_C2Chl/12*data2$QoutInt[1]/data2$epiVol[1]-phyto_C2Chl/12*sedRateVec[i]-phyto_C2Chl/12*mortVec[i]+(data2$entrainVol[1]-data2$streamWaterdisch[1]*(1-splitFunc(data2$epiDens[1],data2$streamDens[1],fracInVec[i])))*data2$entrainHypo[1]/data2$epiVol[1]*phyto_C2Chl/12,0),
                nrow=7,byrow=T)
}
...
y=array(rbind(dicVec,docVec,chlVec,pocVec),dim=c(4,1,nStep))
...
X[5,1,1,]<-X[7,1,1,]-X[6,1,1,]*phyto_C2Chl/12 # tpoc pool is differnce between POC and Chl pools 
...
for(i in 1:nEn){
  C[,,1,i]<-matrix(c(data2$kCO2[1],1,-data2$ma_gpp[1],0,0,0,data2$hypo_dicInt[1],0,
                     ...
                     0,0,0,0,0,0,data2$hypo_pocInt[1]-(data2$hypo_chlInt[1]*phyto_C2Chl/12),1,
                     0,0,data2$ma_gpp[1]*12/phyto_C2Chl,0,0,0,data2$hypo_chlInt[1],0,
...
for(t in 2:nStep){
  for(i in 1:nEn){
    ...
    z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+
                       ...
                       1-phyto_C2Chl/12*data2$QoutInt[t]/data2$epiVol[t]-phyto_C2Chl/12*z$pars[5,1,t,i]-phyto_C2Chl/12*z$pars[6,1,t,i]+(data2$entrainVol[t]-data2$streamWaterdisch[t]*(1-splitFunc(data2$epiDens[t],data2$streamDens[t],z$pars[4,1,t,i])))*data2$entrainHypo[t]/data2$epiVol[t]*phyto_C2Chl/12,0),
                     nrow=7,byrow=T)
    ...
    z$C[,,t,i]<-matrix(c(data2$kCO2[t],1,-data2$ma_gpp[t],0,0,0,data2$hypo_dicInt[t],0,
                       ...
                       0,0,0,0,0,0,data2$hypo_pocInt[t]-(data2$hypo_chlInt[t]*phyto_C2Chl/12),1,
                       0,0,data2$ma_gpp[t]*12/phyto_C2Chl,0,0,0,data2$hypo_chlInt[t],0,
...
ylim=range(Y[10,1,,]/data2$epiVol*12)
...
windows()
sedtPOC<-apply(Y[11,1,,]*Y[5,1,,],MARGIN = 1,FUN=mean)
sedChl<-apply(Y[12,1,,]*Y[5,1,,]+Y[12,1,,]*Y[6,1,,],MARGIN = 1,FUN=mean) # mort + sed 
sedOut<-apply(Y[5,1,,],MARGIN = 1,FUN=mean)
sedtPOC=sedtPOC*12/data2$A0*1000 # mg C m-2 day-1 
sedChl=sedChl*phyto_C2Chl/data2$A0*1000 # mg C m-2 day-1 
sedTot=sedChl+sedtPOC
ylim=range(Y[11,1,,]*Y[5,1,,]*12/data2$A0*1000+Y[12,1,,]*Y[5,1,,]*phyto_C2Chl/data2$A0*1000+Y[12,1,,]*Y[6,1,,]*phyto_C2Chl/data2$A0*1000)
plot(sedTot,type='l',ylim=ylim,ylab='Sedimentation mg C m-2 day-1')
for(i in 1:nEn){
  lines(Y[11,1,,i]*Y[5,1,,i]*12/data2$A0*1000+Y[12,1,,i]*Y[5,1,,i]*phyto_C2Chl/data2$A0*1000+Y[12,1,,i]*Y[6,1,,i]*phyto_C2Chl/data2$A0*1000,col='gray',ylab='')
}
lines(sedTot,lwd=2)
par(new=T)
plot(data2$C_mmol_m2_day*12,col='red',ylim=ylim,pch=16)
