...
source('/Users/Jake/Documents/Jake/MyPapers/Zwart 5/R Code/loadWLData.R')
library(MASS)
library(data.table)
library(R.utils)
...
load('/Users/Jake/Documents/Jake/MyPapers/Zwart 5/R Data/WL_20170429.RData')
...
parGuess <- c(0.002,0.1,0.10,0.9,0.01,0.001) #r20; units: day-1; fraction labile of loaded DOC; fraction inlet that goes into epi; turnover rate parameters set constant frac labile estimated 
...
for(i in 1:nEn){
  ut[,,1,i]<-matrix(c(data2$DICeq[1]*data2$epiVol[1]/data2$thermo.depth[1],
                      ...
                      exude,exudeLabile,data2$docIn[1]-data2$overlandDOCdisch[1]/12/1000*(1-splitFunc(data2$epiDens[1],data2$streamDens[1],fracInVec[i])),
...
for(t in 2:nStep){
  for(i in 1:nEn){
    ...
    z$ut[,,t,i]<-matrix(c(data2$DICeq[t]*data2$epiVol[t]/data2$thermo.depth[t],
                        ...
                        (data2$entrainVol[t]-data2$overlandDOCdisch[t]*(1-splitFunc(data2$epiDens[t],data2$streamDens[t],z$pars[4,1,t,i])))*data2$entrainEpi[t],
...
windows()
Chlout<-apply(z$X[6,1,,],MARGIN = 1,FUN=mean)
ylim=range(z$X[6,1,,]/data2$epiVol*1000) # ug Chl L-1 
plot(Chlout/data2$epiVol*1000,type='l',ylim=ylim,ylab='Chl ug/L')
for(i in 1:nEn){
  lines(z$X[6,1,,i]/data2$epiVol*1000,col='gray',ylab='')
}
lines(Chlout/data2$epiVol*1000,ylab='')
par(new=T)
plot(z$y[3,1,,1]/data2$epiVol*1000,ylim=ylim,col='red',pch=16,ylab='')
arrows(seq(1:nStep),(z$y[3,1,,1]/data2$epiVol*1000)-chlSD*1000,seq(1:nStep),(z$y[3,1,,1]/data2$epiVol*1000)+chlSD*1000,code=3,length=0.1,angle=90,col='red')
...
sedChl=sedChl*phyto_C2Chl/data2$A0*1000 # mg C m-2 day-1 
windows()
...
windows()
ylim=range(Y[11,1,,]*Y[5,1,,]*12/data2$A0*1000+Y[12,1,,]*Y[5,1,,]*phyto_C2Chl/data2$A0*1000)
plot(sedTot,type='l',ylim=ylim,ylab='Sedimentation mg C m-2 day-1')
for(i in 1:nEn){
  lines(Y[11,1,,i]*Y[5,1,,i]*12/data2$A0*1000+Y[12,1,,i]*Y[5,1,,i]*phyto_C2Chl/data2$A0*1000,col='gray',ylab='')
}
lines(sedTot,lwd=2)
par(new=T)
plot(data2$C_mmol_m2_day*12,col='red',ylim=ylim,pch=16)
