...
for(i in 1:length(years)){
  for(j in 1:length(site)){
    ...
    colnames(curCO2)<-c('datetime','Inlet_co2')
    curCO2$Inlet_co2<-curCO2$Inlet_co2/1000/1000*1000 #converting from umol L-1 to mol C m-3 
    ...
    colnames(curDIC)<-c('datetime','Inlet_dic')
    curDIC$Inlet_dic<-curDIC$Inlet_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3 
    ...
    cur$Inlet_co2[1]<-cur$Inlet_co2[min(which(!is.na(cur$Inlet_co2)))]
    cur$Inlet_co2[nrow(cur)]<-cur$Inlet_co2[max(which(!is.na(cur$Inlet_co2)))]
    cur$Inlet_co2<-approx(as.numeric(rownames(cur)),cur$Inlet_co2,as.numeric(rownames(cur)))$y
    cur$co2discharge<-cur$dischargetotal*cur$Inlet_co2 # mol C / 10 min 
    
    cur<-merge(cur,curDIC,by='datetime',all.x = T)
    cur$Inlet_dic[1]<-cur$Inlet_dic[min(which(!is.na(cur$Inlet_dic)))]
    cur$Inlet_dic[nrow(cur)]<-cur$Inlet_dic[max(which(!is.na(cur$Inlet_dic)))]
    cur$Inlet_dic<-approx(as.numeric(rownames(cur)),cur$Inlet_dic,as.numeric(rownames(cur)))$y
    cur$dicdischarge<-cur$dischargetotal*cur$Inlet_dic # mol C / 10 min 
    ...
    colnames(cur)<-c('datetime','docdischarge','tpdischarge','dischargetotal','pocdischarge','co2discharge','dicdischarge','inlet.temp')
    data<-rbind(data,cur)
  }
}
...
dataTemp<-aggregate(data$inlet.temp,by=list(data$datetime),FUN=mean)
...
data$gwDischTP<-data$gwDisch*10*1000
data$gwDischDOC<-data$gwDisch*13*1000 #total amount of DOC entering in GW mg/day
data$gwDischDIC<-data$gwDisch*8.43*1000 # total amount of DIC entering in GW mg/day; 8.43 mg/L is median of NTL data 
...
metab2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/MO/MO_2015 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/MO/MO_2015 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
...
gc$depthTop<-as.numeric(gc$depthTop)
gc$depthBottom<-as.numeric(gc$depthBottom)
moDeepholeCO2<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'MO deepHole point pCO2'&gc$depthTop<0.9&gc$CO2original_umolL>2,]
moDeepholeDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'MO deepHole point DIC'&gc$depthBottom<0.9&gc$CO2original_umolL>30,]
moHypoDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                'MO deepHole point DIC'&gc$depthTop>3&gc$CO2original_umolL>600,]

moDeepholeCO2<-aggregate(moDeepholeCO2$CO2original_umolL~moDeepholeCO2$dateSample,FUN=mean)
colnames(moDeepholeCO2)<-c('datetime','co2') 
moDeepholeCO2$co2<-moDeepholeCO2$co2/1000/1000*1000 #converting from umol L-1 to mol C m-3
moDeepholeDIC<-aggregate(moDeepholeDIC$CO2original_umolL~moDeepholeDIC$dateSample,FUN=mean)
colnames(moDeepholeDIC)<-c('datetime','dic') 
moDeepholeDIC$dic<-moDeepholeDIC$dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
moHypoDIC<-aggregate(moHypoDIC$CO2original_umolL~moHypoDIC$dateSample,FUN=mean)
colnames(moHypoDIC)<-c('datetime','hypo_dic') 
moHypoDIC$hypo_dic<-moHypoDIC$hypo_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
data<-merge(data,moDeepholeCO2,by='datetime',all.x=T)
data<-merge(data,moDeepholeDIC,by='datetime',all.x=T)
data<-merge(data,moHypoDIC,by='datetime',all.x=T)
...
data$dicIn<-data$streamDICdisch+data$precipDIC/12/1000+data$gwDischDIC/12/1000+data$overland*(data$streamDICdisch/data$streamWaterdisch) # mol C day-1 
...
highFreqStaff<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/PressureSensorWaterHeight/MO/StaffGauge/MO_StaffGauge_waterHeight.txt',
                          stringsAsFactors = F,sep='\t',header=T)
highFreqStaff15<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/PressureSensorWaterHeight/MO/WholeLake/MO_WholeLake_waterHeight.txt',
                            stringsAsFactors = F,sep='\t',header=T)
...
# sedimentation rates 
sedTrap2014<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Sediment Trap CNP/MO/MO_SedTrap.csv',stringsAsFactors = F)
sedTrap2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Sediment Trap CNP/MO/MO_SedTrap.csv',stringsAsFactors = F)
...
data$pocIn=data$tPOC_dep+data$streamPOCdisch/1000/12 # mol C day-1 POC 
