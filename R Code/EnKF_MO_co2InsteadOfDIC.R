...
parGuess <- c(0.004,0.1,0.30,0.9,0.06,0.001) #r20; units: day-1; fraction labile of loaded DOC; fraction inlet that goes into epi; turnover rate parameters set constant frac labile estimated 
...
X[6,1,1,]<-abs(rnorm(n=nEn,y[3,1,1,],sd=chlPoolSD[1])) #chl pool - forced positive; g Chl in epi 
...
for(t in 2:nStep){
  for(i in 1:nEn){
    ...
    z$B[,,t,i]<-matrix(c(1-data2$QoutInt[t]/data2$epiVol[t]-data2$kCO2[t]/data2$thermo.depth[t]+
                       ...
                       0,0,0,0,1-data2$QoutInt[t]/data2$epiVol[t]-z$pars[5,1,t,i]+(data2$entrainVol[t]-data2$streamWaterdisch[t]*(1-splitFunc(data2$epiDens[t],data2$streamDens[t],z$pars[4,1,t,i])))*data2$entrainHypo[t]/data2$epiVol[t],0,0,
                       0,0,0,0,0,1-data2$QoutInt[t]/data2$epiVol[t]-z$pars[5,1,t,i]-z$pars[6,1,t,i]+(data2$entrainVol[t]-data2$streamWaterdisch[t]*(1-splitFunc(data2$epiDens[t],data2$streamDens[t],z$pars[4,1,t,i])))*data2$entrainHypo[t]/data2$epiVol[t],0,
                       0,0,0,0,1-data2$QoutInt[t]/data2$epiVol[t]-z$pars[5,1,t,i]+(data2$entrainVol[t]-data2$streamWaterdisch[t]*(1-splitFunc(data2$epiDens[t],data2$streamDens[t],z$pars[4,1,t,i])))*data2$entrainHypo[t]/data2$epiVol[t],
