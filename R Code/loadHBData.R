...
data$gwDisch<-rep(10,length(data$datetime)) #m3/day
...
years<-c(2014,2015)
metab2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/Results/HB/HB_2014 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/LakeMetabolism/Results/HB/HB_2014 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
...
metab2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/HB/HB_2015 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/HB/HB_2015 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
...
if(lake%in%c('EL','MO','HB','WL')){
  ...
  crData$overland<-crData$overland/perim$perim[perim$lakeID=='CR']*perim$perim[perim$lakeID==lake]*.3 # m3 / m shoreline / day 
  ...
  data$overland<-ifelse(is.na(data$overland),0,data$overland)
  data$overlandDOCdisch<-data$overland*36*1000
...
hbDeepholeCO2<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'HB deepHole point pCO2'&gc$depthTop<0.9&gc$CO2original_umolL>7&gc$CO2original_umolL<1000,]
hbDeepholeDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'HB deepHole point DIC'&gc$depthBottom<0.9&gc$CO2original_umolL>17,]
hbHypoDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                'HB deepHole point DIC'&gc$depthTop>3&gc$CO2original_umolL>6,]
...
hbDeepholeCO2<-aggregate(hbDeepholeCO2$CO2original_umolL~hbDeepholeCO2$dateSample,FUN=mean)
colnames(hbDeepholeCO2)<-c('datetime','co2') 
hbDeepholeCO2$co2<-hbDeepholeCO2$co2/1000/1000*1000 #converting from umol L-1 to mol C m-3
hbDeepholeDIC<-aggregate(hbDeepholeDIC$CO2original_umolL~hbDeepholeDIC$dateSample,FUN=mean)
colnames(hbDeepholeDIC)<-c('datetime','dic') 
hbDeepholeDIC$dic<-hbDeepholeDIC$dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
hbHypoDIC<-aggregate(hbHypoDIC$CO2original_umolL~hbHypoDIC$dateSample,FUN=mean)
colnames(hbHypoDIC)<-c('datetime','hypo_dic') 
hbHypoDIC$hypo_dic<-hbHypoDIC$hypo_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
data<-merge(data,hbDeepholeCO2,by='datetime',all.x=T)
data<-merge(data,hbDeepholeDIC,by='datetime',all.x=T)
data<-merge(data,hbHypoDIC,by='datetime',all.x=T)
...
highFreqStaff<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/PressureSensorWaterHeight/HB/StaffGauge/HB_StaffGauge_waterHeight.txt',
                          stringsAsFactors = F,sep='\t',header=T)
highFreqStaff15<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/PressureSensorWaterHeight/HB/WholeLake/HB_WholeLake_waterHeight.txt',
                            stringsAsFactors = F,sep='\t',header=T)
...
# sedimentation rates 
sedTrap2014<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Sediment Trap CNP/HB/HB_SedTrap.csv',stringsAsFactors = F)
sedTrap2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Sediment Trap CNP/HB/HB_SedTrap.csv',stringsAsFactors = F)
