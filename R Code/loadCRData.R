...
staff<-dbTable('staff_gauges',lakeID=lake,dbname = 'M2Mdb_041614.db')
...
doc<-dbTable('doc',lakeID=lake,depthClass='PML',dbname = 'M2Mdb_041614.db')
...
poc2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/Data/Limnology/POC_PON_2014.txt',stringsAsFactors = F,header = T,sep='\t')
poc2014<-poc2014[poc2014$Lake.ID==lake&poc2014$Depth.class=='PML'&poc2014$flag==0&poc2014$Sample.Type=='filter',]
poc2014<-poc2014[!is.na(poc2014$POC),]
poc2014$Date.Sample<-strftime(strptime(poc2014$Date.Sample,'%m/%d/%Y'),'%Y-%m-%d')
poc2014<-aggregate(poc2014$POC,by=list(poc2014$Date.Sample),FUN=mean)
colnames(poc2014)<-c('datetime','poc')
poc2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/POC2015_samps.csv',stringsAsFactors = F)
poc2015<-poc2015[poc2015$lakeID==lake&poc2015$depthClass=='PML'&poc2015$sampleType=='Filter',]
poc2015<-poc2015[!is.na(poc2015$POC),]
poc2015$dateSample<-strftime(strptime(poc2015$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
poc2015<-aggregate(poc2015$POC,by=list(poc2015$dateSample),FUN=mean)
colnames(poc2015)<-c('datetime','poc')
poc<-rbind(poc2014,poc2015)
...
source('/Users/Jake/Desktop/R functions/dbTable.R')
tp<-dbTable('nutrients',lakeID = lake,dbname = 'M2Mdb_041614.db') #function that grabs table from database 
...
poc2014_st<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/Data/Limnology/POC_PON_2014.txt',stringsAsFactors = F,header = T,sep='\t')
poc2014_st$Site<-ifelse(poc2014_st$Site=='Inlet 1','Inlet1',poc2014_st$Site)
poc2014_st$Site<-ifelse(poc2014_st$Site=='Inlet ','Inlet1',poc2014_st$Site)
poc2014_st$Site<-ifelse(poc2014_st$Site=='Inlet','Inlet1',poc2014_st$Site)
poc2014_st$Site<-ifelse(poc2014_st$Site=='Inlet 2','Inlet2',poc2014_st$Site)
poc2014_st$Site<-ifelse(poc2014_st$Site=='Inlet 3','Inlet3',poc2014_st$Site)
poc2014_st<-poc2014_st[poc2014_st$Lake.ID==lake&poc2014_st$Site%in%site&poc2014_st$flag==0&poc2014_st$Sample.Type=='filter',]
poc2014_st<-poc2014_st[!is.na(poc2014_st$POC),]
poc2014_st$Date.Sample<-strftime(strptime(poc2014_st$Date.Sample,'%m/%d/%Y'),'%Y-%m-%d')
poc2015_st<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/POC2015_samps.csv',stringsAsFactors = F)
poc2015_st$siteName<-ifelse(poc2015_st$siteName=='Inlet','Inlet1',poc2015_st$siteName)
poc2015_st<-poc2015_st[poc2015_st$lakeID==lake&poc2015_st$siteName%in%site&poc2015_st$sampleType=='Filter',]
poc2015_st<-poc2015_st[!is.na(poc2015_st$POC),]
poc2015_st$dateSample<-strftime(strptime(poc2015_st$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
poc2015_st<-poc2015_st[poc2015_st$POC<10,]
...
for(i in 1:length(years)){
  for(j in 1:length(site)){
    ...
    curPOC=poc2014_st[poc2014_st$Site==site[j],]
    curPOC<-aggregate(curPOC$POC,by=list(curPOC$Date.Sample),FUN=mean)
    colnames(curPOC)<-c('datetime','poc_st')
    curPOC$datetime<-as.POSIXct(curPOC$datetime,'%Y-%m-%d',tz='GMT')
    # gc data CO2 loading 
    curCO2<-gc[paste(gc$lakeID,gc$siteName,gc$subsampleClass)==paste(lake,site[j],'pCO2'),]
    curDIC<-gc[paste(gc$lakeID,gc$siteName,gc$subsampleClass)==paste(lake,site[j],'DIC'),]
    if(length(grep('bioassay',tolower(curDIC$runName)))>0){
      curDIC<-curDIC[-grep('bioassay',tolower(curDIC$runName)),]
    }
    if(length(grep('bioassay',tolower(curCO2$runName)))>0){
      curCO2<-curCO2[-grep('bioassay',tolower(curCO2$runName)),]
    }
    curCO2<-aggregate(curCO2$CO2original_umolL~curCO2$dateSample,FUN=mean)
    colnames(curCO2)<-c('datetime','Outlet_co2')
    curCO2$Outlet_co2<-curCO2$Outlet_co2/1000/1000*1000 #converting from umol L-1 to mol C m-3 
    curCO2$datetime<-as.POSIXct(curCO2$datetime,'%Y-%m-%d',tz = 'GMT')
    curDIC<-aggregate(curDIC$CO2original_umolL~curDIC$dateSample,FUN=mean)
    colnames(curDIC)<-c('datetime','Outlet_dic')
    curDIC$Outlet_dic<-curDIC$Outlet_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3 
    curDIC$datetime<-as.POSIXct(curDIC$datetime,'%Y-%m-%d',tz='GMT')
    
    cur<-merge(cur,curPOC,by='datetime',all.x=T)
    cur$poc_st[1]<-cur$poc_st[min(which(!is.na(cur$poc_st)))]
    cur$poc_st[nrow(cur)]<-cur$poc_st[max(which(!is.na(cur$poc_st)))]
    cur$poc_st<-approx(as.numeric(rownames(cur)),cur$poc_st,as.numeric(rownames(cur)))$y
    cur$pocdischarge<-cur$dischargetotal*cur$poc_st*1000 # mg C / 10 min 
    
    cur<-merge(cur,curCO2,by='datetime',all.x = T)
    cur$Outlet_co2[1]<-cur$Outlet_co2[min(which(!is.na(cur$Outlet_co2)))]
    cur$Outlet_co2[nrow(cur)]<-cur$Outlet_co2[max(which(!is.na(cur$Outlet_co2)))]
    cur$Outlet_co2<-approx(as.numeric(rownames(cur)),cur$Outlet_co2,as.numeric(rownames(cur)))$y
    cur$co2discharge<-cur$dischargetotal*cur$Outlet_co2 # mol C / 10 min 
    
    cur<-merge(cur,curDIC,by='datetime',all.x = T)
    cur$Outlet_dic[1]<-cur$Outlet_dic[min(which(!is.na(cur$Outlet_dic)))]
    cur$Outlet_dic[nrow(cur)]<-cur$Outlet_dic[max(which(!is.na(cur$Outlet_dic)))]
    cur$Outlet_dic<-approx(as.numeric(rownames(cur)),cur$Outlet_dic,as.numeric(rownames(cur)))$y
    cur$dicdischarge<-cur$dischargetotal*cur$Outlet_dic # mol C / 10 min 
    
    cur<-cur[,c('datetime','docdischarge','tpdischarge','dischargetotal','pocdischarge','co2discharge','dicdischarge',paste(tolower(site[j]),'.temp',sep=''))]
    colnames(cur)<-c('datetime','docdischarge','tpdischarge','dischargetotal','pocdischarge','co2discharge','dicdischarge','outlet.temp')
    data<-rbind(data,cur)
  }
}
dir<-'/Users/Jake/Documents/Jake/UNDERC 2015/StreamDischarge/'
for(j in 1:length(site)){
  cur<-read.csv(paste(dir,'/',lake,'/',site[j],'/',lake,site[j],'Discharge.csv',sep=''))
  colnames(cur)<-tolower(colnames(cur))
  cur$datetime<-as.POSIXct(cur$datetime)
  curPOC=poc2015_st[poc2015_st$siteName==site[j],]
  curPOC<-aggregate(curPOC$POC,by=list(curPOC$dateSample),FUN=mean)
  colnames(curPOC)<-c('datetime','poc_st')
  curPOC$datetime<-as.POSIXct(curPOC$datetime,'%Y-%m-%d',tz='GMT')
  # gc data CO2 loading 
  curCO2<-gc[paste(gc$lakeID,gc$siteName,gc$subsampleClass)==paste(lake,site[j],'pCO2'),]
...
dataTemp<-aggregate(data$outlet.temp,by=list(data$datetime),FUN=mean)
dataPOC<-aggregate(data$pocdischarge,by=list(data$datetime),FUN=sum)
dataCO2<-aggregate(data$co2discharge,by=list(data$datetime),FUN=sum)
dataDIC<-aggregate(data$dicdischarge,by=list(data$datetime),FUN=sum)
...
colnames(dataPOC)<-c('datetime','streamPOCdisch')
colnames(dataCO2)<-c('datetime','streamCO2disch')
colnames(dataDIC)<-c('datetime','streamDICdisch')
...
dataPOC<-dataPOC[sort.list(dataPOC$datetime),]
dataCO2<-dataCO2[sort.list(dataCO2$datetime),]
dataDIC<-dataDIC[sort.list(dataDIC$datetime),]
...
colnames(dataTemp)<-c('datetime','streamTemp')
dataPOC$datetime<-strftime(strptime(dataPOC$datetime,'%Y-%m-%d %H:%M:%S'),'%Y-%m-%d')
dataPOC<-aggregate(dataPOC$streamPOCdisch,by=list(dataPOC$datetime),FUN=sum)
colnames(dataPOC)<-c('datetime','streamPOCdisch')
dataCO2$datetime<-strftime(strptime(dataCO2$datetime,'%Y-%m-%d %H:%M:%S'),'%Y-%m-%d')
dataCO2<-aggregate(dataCO2$streamCO2disch,by=list(dataCO2$datetime),FUN=sum)
colnames(dataCO2)<-c('datetime','streamCO2disch')
dataDIC$datetime<-strftime(strptime(dataDIC$datetime,'%Y-%m-%d %H:%M:%S'),'%Y-%m-%d')
dataDIC<-aggregate(dataDIC$streamDICdisch,by=list(dataDIC$datetime),FUN=sum)
colnames(dataDIC)<-c('datetime','streamDICdisch')

data<-merge(dataWater,dataDOC,by='datetime')
data<-merge(data,dataPOC,by='datetime')
data<-merge(data,dataCO2,by='datetime')
data<-merge(data,dataDIC,by='datetime')
...
data<-merge(data,poc,by='datetime',all.x=T)
...
data$precipDIC<-data$precip*1000*1 # total amount of DIC from precip in mg/day - DIC is from Cardille et al. 2007 ; also see Stets et al 2009 =>1.5 mg L-1
...
data$waterHeight_m_Int<-data$waterHeight_m
data$waterHeight_m_Int[1]<-data$waterHeight_m_Int[min(which(!is.na(data$waterHeight_m_Int)))] # making first staff gauge first non-na 
data$waterHeight_m_Int[length(data$datetime)]<-data$waterHeight_m_Int[max(which(!is.na(data$waterHeight_m_Int)))] # making last staff gauge last non-na 
data$waterHeight_m_Int<-approx(x=as.Date(data$datetime),y=data$waterHeight_m_Int,xout=as.Date(data$datetime))$y
#change in storage term for DOC 
data$delta_storage<-rep(NA,length(data$datetime))
for(i in 1:(length(data$datetime)-1)){
  data$delta_storage[i]<-((data$waterHeight_m_Int[i+1]-data$waterHeight_m_Int[i])*lakeArea) # change in volume m3 
}
data$delta_storage[length(data$dateTime)]<-0
...
if(lake=='EL'){
  ...
  Stage0<-data$waterHeight_m[min(which(!is.na(data$waterHeight_m)))] #initial stage height in m (all water added to the lake will be in reference to this height)
...
metab2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/CR/CR_2015 optimOut.txt',
                      sep='',stringsAsFactors = F,header=T)
gpp2015<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/LakeMetabolism/Results/Pelagic/CR/CR_2015 GPPFitOut.txt',
                    sep='',stringsAsFactors = F, header = T)
...
epiTemp<-data.frame()
for(i in 1:length(years)){
  cur<-read.table(file.path('/Users/Jake/Documents/Jake/MyPapers/Zwart 5/Data/',years[i],lake,'TEMP_PROF.txt'),
  ...
  curZ$thermo.depth<-ifelse(is.na(curZ$thermo.depth),max(get.offsets(cur)),curZ$thermo.depth)
  curMix$z.mix<-ifelse(is.na(curMix$z.mix),max(get.offsets(cur)),curMix$z.mix)
...
epiTemp$epiTemp<-ifelse(is.infinite(epiTemp$epiTemp),NA,epiTemp$epiTemp)
epiTemp$epiTemp[1]<-epiTemp$epiTemp[min(which(!is.na(epiTemp$epiTemp)))] 
epiTemp$epiTemp[nrow(epiTemp)]<-epiTemp$epiTemp[max(which(!is.na(epiTemp$epiTemp)))] 
epiTemp$epiTemp<-approx(as.numeric(rownames(epiTemp)),epiTemp$epiTemp,as.numeric(rownames(epiTemp)))$y
...
if(lake=='CR'){
  data$V0<-as.numeric(bathy$volumeToBottom3D_m3[1]) #m3 lake Volume original from database 
  data$A0<-as.numeric(bathy$area_m2[1]) #m2 lake area original from database 
}else{
...
gc$depthTop<-as.numeric(gc$depthTop)
gc$depthBottom<-as.numeric(gc$depthBottom)
crDeepholeCO2<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'CR deepHole point pCO2'&gc$depthTop<0.9&gc$CO2original_umolL>6&gc$CO2original_umolL<100,]
crDeepholeDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                    'CR deepHole point DIC'&gc$depthBottom<0.9&gc$CO2original_umolL>4,]
crHypoDIC<-gc[paste(gc$lakeID,gc$siteName,gc$depthClass,gc$subsampleClass)==
                'CR deepHole point DIC'&gc$depthTop>3&gc$CO2original_umolL>3,]

crDeepholeCO2<-aggregate(crDeepholeCO2$CO2original_umolL~crDeepholeCO2$dateSample,FUN=mean)
colnames(crDeepholeCO2)<-c('datetime','co2') 
crDeepholeCO2$co2<-crDeepholeCO2$co2/1000/1000*1000 #converting from umol L-1 to mol C m-3
crDeepholeDIC<-aggregate(crDeepholeDIC$CO2original_umolL~crDeepholeDIC$dateSample,FUN=mean)
colnames(crDeepholeDIC)<-c('datetime','dic') 
crDeepholeDIC$dic<-crDeepholeDIC$dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
crHypoDIC<-aggregate(crHypoDIC$CO2original_umolL~crHypoDIC$dateSample,FUN=mean)
colnames(crHypoDIC)<-c('datetime','hypo_dic') 
crHypoDIC$hypo_dic<-crHypoDIC$hypo_dic/1000/1000*1000 #converting from umol L-1 to mol C m-3
data<-merge(data,crDeepholeCO2,by='datetime',all.x=T)
data<-merge(data,crDeepholeDIC,by='datetime',all.x=T)
data<-merge(data,crHypoDIC,by='datetime',all.x=T)

data$outletDischarge<-data$streamWaterdisch # m3 day-1 
# find lake DIC data ************************ 
...
# data$co2In<-data$streamCO2disch # mol C day-1 
# data$dicIn<-data$streamDICdisch # mol C day-1 
data$co2In<-data$overland*(data$streamCO2disch/data$streamWaterdisch) # m3 day-1 x mol C day-1 / m3 day-1 = mol C day-1 
data$co2In<-ifelse(is.na(data$co2In),0,data$co2In)
data$dicIn<-data$overland*(data$streamDICdisch/data$streamWaterdisch)+data$precipDIC/12/1000 # m3 day-1 x mol C day-1 / m3 day-1 = mol C day-1 
data$dicIn<-ifelse(is.na(data$dicIn),0,data$dicIn)
...
highFreqStaff<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/PressureSensorWaterHeight/CR/StaffGauge/CR_StaffGauge_waterHeight.txt',
                          stringsAsFactors = F,sep='\t',header=T)
highFreqStaff15<-read.table('/Users/Jake/Documents/Jake/UNDERC 2015/PressureSensorWaterHeight/CR/WholeLake/CR_WholeLake_waterHeight.txt',
                            stringsAsFactors = F,sep='\t',header=T)
...
data$outletDischarge<-ifelse(data$outletDischarge<0,0,data$outletDischarge)

data$Qout<-data$outletDischarge # water discharge out m3 day-1 
...
data$waterLoad<-data$streamWaterdisch+data$precip+data$overland+data$gwDisch # m3 day-1
...
# hypolimnion DOC, DIC, POC, and chl entrainment 
doc2014<-read.csv('/Users/Jake/Documents/Jake/MyPapers/Pulses and Lags/Data/DOC2014.csv',stringsAsFactor=F)
doc2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/DOC2015.csv',stringsAsFactors = F)
...
poc2014<-read.table('/Users/Jake/Documents/Jake/UNDERC 2014/Data/Limnology/POC_PON_2014.txt',stringsAsFactors = F,header = T,sep='\t')
poc2014<-poc2014[poc2014$Lake.ID==lake&poc2014$Depth.class=='Hypo'&poc2014$flag==0&poc2014$Sample.Type=='filter',]
poc2014<-poc2014[!is.na(poc2014$POC),]
poc2014$Date.Sample<-strftime(strptime(poc2014$Date.Sample,'%m/%d/%Y'),'%Y-%m-%d')
poc2014<-aggregate(poc2014$POC,by=list(poc2014$Date.Sample),FUN=mean)
colnames(poc2014)<-c('datetime','hypo_poc')
poc2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/POC2015_samps.csv',stringsAsFactors = F)
poc2015<-poc2015[poc2015$lakeID==lake&poc2015$depthClass=='Hypo'&poc2015$sampleType=='Filter',]
poc2015<-poc2015[!is.na(poc2015$POC),]
poc2015$dateSample<-strftime(strptime(poc2015$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
poc2015<-aggregate(poc2015$POC,by=list(poc2015$dateSample),FUN=mean)
colnames(poc2015)<-c('datetime','hypo_poc')
poc<-rbind(poc2014,poc2015)
data<-merge(data,poc,by='datetime',all.x=T)
data$hypo_pocInt<-data$hypo_poc
data$hypo_pocInt[1]<-data$hypo_pocInt[min(which(!is.na(data$hypo_pocInt)))]
data$hypo_pocInt[length(data$hypo_pocInt)]<-data$hypo_pocInt[max(which(!is.na(data$hypo_pocInt)))]
data$hypo_pocInt<-approx(data$timeStep,data$hypo_pocInt,data$timeStep)$y

# sedimentation rates 
sedTrap2014<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Sediment Trap CNP/CR/CR_SedTrap.csv',stringsAsFactors = F)
sedTrap2015<-read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Sediment Trap CNP/CR/CR_SedTrap.csv',stringsAsFactors = F)
sedTrap2014=sedTrap2014[,colnames(sedTrap2014)%in%colnames(sedTrap2015)]
sedTrap2015=sedTrap2015[,colnames(sedTrap2015)%in%colnames(sedTrap2014)]

sedAll=rbind(sedTrap2014,sedTrap2015)
# use midpoint of datetimeset and retrieved as the merge date 
sedAll$datetimeMid<-as.Date((as.POSIXct(sedAll$datetimeRetrieved)-as.POSIXct(sedAll$datetimeSet))/2+as.POSIXct(sedAll$datetimeSet))
meanCsed=aggregate(sedAll$C_mmol_m2_day~sedAll$datetimeMid,FUN = mean)
colnames(meanCsed)<-c('datetime','C_mmol_m2_day')
meanCsed$datetime<-as.character(meanCsed$datetime)
data<-merge(data,meanCsed,all.x=T)
# interpolating sedimentation rates 
data$C_mmol_m2_day_Int<-data$C_mmol_m2_day
data$C_mmol_m2_day_Int[1]<-data$C_mmol_m2_day_Int[min(which(!is.na(data$C_mmol_m2_day_Int)))]
data$C_mmol_m2_day_Int[length(data$C_mmol_m2_day_Int)]<-data$C_mmol_m2_day_Int[max(which(!is.na(data$C_mmol_m2_day_Int)))]
data$C_mmol_m2_day_Int<-approx(data$timeStep,data$C_mmol_m2_day_Int,xout = data$timeStep)$y

# tPOC deposition and total tPOC flux 
...
dailyTPOCdep<-function(days,Area,directPrecip){
  tPOCdepTotal<-rep(NA,length(days))
  for(i in 1:length(days)){
    maxWind=data$wnd_10[i]*(1/10)^0.15		# convert 10m wind to 1m for preston model tPOC deposition
    curArea=Area[i]
    curDirectPrecip=directPrecip[i]
    
    tPOCdepTotal_cur=integrate(depWithDist,0.01,sqrt(curArea/pi),precip=(curDirectPrecip/curArea/1000),maxWind=maxWind)$value/sqrt(curArea/pi)*curArea #tPOCdep summed across lake surface	#mol C day-1
    tPOCdepTotal[i]<-tPOCdepTotal_cur
  }
  
  return(tPOCdepTotal)
}
deposition=dailyTPOCdep(data$datetime,data$A0,data$precip)    

data$tPOC_dep=deposition # mol C day-1 
data$pocIn=data$tPOC_dep # mol C day-1 POC 

# chlorophyll-a 
chl14=read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Data/Limnology/Chlorophyll2014_samp.csv',stringsAsFactors = F)
chl15=read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/Chlorophyll2015_samp.csv',stringsAsFactors = F)
chl14<-chl14[chl14$lakeID==lake&chl14$depthClass=='PML'&chl14$flag==0,]
chl14$datetime<-strftime(strptime(chl14$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
chl14<-aggregate(chl14$chl,by=list(chl14$datetime),FUN=mean)
colnames(chl14)<-c('datetime','chl')
chl15<-chl15[chl15$lakeID==lake&chl15$depthClass=='PML'&chl15$flag==0,]
chl15$datetime<-strftime(strptime(chl15$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
chl15<-aggregate(chl15$chl,by=list(chl15$datetime),FUN=mean)
colnames(chl15)<-c('datetime','chl')
chl=rbind(chl14,chl15) # ug chl-a L-1
chl<-chl[chl$chl>0,]

chl14=read.csv('/Users/Jake/Documents/Jake/UNDERC 2014/Data/Limnology/Chlorophyll2014_samp.csv',stringsAsFactors = F)
chl15=read.csv('/Users/Jake/Documents/Jake/UNDERC 2015/Data/Limnology/Chlorophyll2015_samp.csv',stringsAsFactors = F)
chl14<-chl14[chl14$lakeID==lake&chl14$depthClass=='Hypo'&chl14$flag==0,]
chl14$datetime<-strftime(strptime(chl14$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
chl14<-aggregate(chl14$chl,by=list(chl14$datetime),FUN=mean)
colnames(chl14)<-c('datetime','hypo_chl')
chl15<-chl15[chl15$lakeID==lake&chl15$depthClass=='Hypo'&chl15$flag==0,]
chl15$datetime<-strftime(strptime(chl15$dateSample,'%m/%d/%Y'),'%Y-%m-%d')
chl15<-aggregate(chl15$chl,by=list(chl15$datetime),FUN=mean)
colnames(chl15)<-c('datetime','hypo_chl')
chl_hypo=rbind(chl14,chl15) # ug chl-a L-1
chl_hypo<-chl_hypo[chl_hypo$hypo_chl<20,]

data<-merge(data,chl,by='datetime',all.x=T)
data<-merge(data,chl_hypo,by='datetime',all.x=T)
# interpolating chl hypo for driving data 
data$hypo_chlInt<-data$hypo_chl
data$hypo_chlInt[1]<-data$hypo_chlInt[min(which(!is.na(data$hypo_chlInt)))]
data$hypo_chlInt[length(data$hypo_chlInt)]<-data$hypo_chlInt[max(which(!is.na(data$hypo_chlInt)))]
data$hypo_chlInt<-approx(data$timeStep,data$hypo_chlInt,data$timeStep)$y
